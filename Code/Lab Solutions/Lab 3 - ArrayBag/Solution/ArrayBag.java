...
public final class ArrayBag<T> implements BagInterface<T> {
	...
	public T remove() {
		...
		if (numberOfEntries > 0) {            
			result = removeEntry(numberOfEntries-1);
		} // end if
		return result;
	} // end remove
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	public void removeDuplicates() {
		...
		// COMPLETE THIS METHOD 
		//>>>>>> ADDED CODE >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		// scan over the list removing duplicates from after our position
		// Note that we need to check against the current size of the bag
		// since it will change as the duplicates are removed
		for ( int i = 0; i<numberOfEntries; i++){
			int after = i + 1;
			if(bag[i] != null) {
				while( after<numberOfEntries){
					if(bag[i].equals(bag[after])){
						// we found a duplicate to remove
						// Copy the last item in the list to the location of the
						// duplicate and then delete the last itme in the list
						bag[after]=bag[numberOfEntries-1];
						bag[numberOfEntries-1]=null;
						numberOfEntries--;
						// IMPORTANT: We don't update "after" since it now points to what once was the
						// last item in bag
					} else {
						// not a duplicate, check the next one
						after++;
					}
				}    
			}
